...
INT_PTR CALLBACK Tab4DialogProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        PathCombine(szDefaultPath, szModulePath, _T("StudyProgress.txt"));
        // 加载文件（关键：只在初始化时加载一次）
        LoadFile(hWnd, IDC_TEXT_EDIT3, szDefaultPath);
        return TRUE;
    }
    ...
    case WM_SIZE:
    {
        ...
        // 例如调整编辑框的大小
        HWND hEdit = GetDlgItem(hWnd, IDC_TEXT_EDIT3);
        if (hEdit && IsWindow(hEdit))
        {
            ...
            // 调整按钮位置
            HWND hButton = GetDlgItem(hWnd, IDC_SAVE_BUTTON3);
    ...
    case WM_COMMAND:
    {
        ...
        if (wmId == IDC_SAVE_BUTTON3)
        {

            // 获取文件加载状态
            BOOL bFileLoaded = (BOOL)(INT_PTR)GetProp(hWnd, PROP_FILE_LOADED);
            hEdit = GetDlgItem(hWnd, IDC_TEXT_EDIT3);
            ...
            if (bFileLoaded)
            {
                if (SaveFile(hWnd, IDC_TEXT_EDIT3, szDefaultPath))
                {
                    MessageBox(hWnd, L"文本已保存到 StudyProgress.txt", L"保存成功", MB_OK | MB_ICONINFORMATION);
                }
                else
                {
            ...
            else
            {
                ...
                {
                    if (SaveFile(hWnd, IDC_TEXT_EDIT3, ofn.lpstrFile))
                    {
