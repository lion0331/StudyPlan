...
INT_PTR CALLBACK Tab1DialogProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    static HWND hEdit;
    ...
    case WM_INITDIALOG:
    {
        ...
        PathCombine(szDefaultPath, szModulePath, _T("StudyGoals.txt"));
        // 加载文件（关键：只在初始化时加载一次）
        LoadFile(hWnd, IDC_TEXT_EDIT, szDefaultPath);
        return TRUE;
    }
    ...
    case WM_SIZE:
    {
        ...
        // 例如调整编辑框的大小
        hEdit = GetDlgItem(hWnd, IDC_TEXT_EDIT);
        if (hEdit && IsWindow(hEdit))
        {
            ...
            // 调整按钮位置
            HWND hButton = GetDlgItem(hWnd, IDC_SAVE_BUTTON);
    ...
    case WM_COMMAND:
    {
        ...
        if (wmId == IDC_SAVE_BUTTON)
        {
            ...
            if (bFileLoaded)
            {
                if (SaveFile(hWnd, IDC_TEXT_EDIT, szDefaultPath))
                {
                    MessageBox(hWnd, L"文本已保存到 StudyGoals.txt", L"保存成功", MB_OK | MB_ICONINFORMATION);
                }
                else
                {
            ...
            else
            {
                ...
                {
                    if (SaveFile(hWnd, IDC_TEXT_EDIT,ofn.lpstrFile))
                    {
